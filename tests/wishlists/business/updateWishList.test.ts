...
} from "../../../src/dtos/wishlist/updateWishList.dto";
...
describe("WishlistBusiness - updateWishlist", () => {
  ...
  test("should update an existing wishlist successfully", async () => {
    ...
    mockWishlistDatabase.findWishlistItemsByWishlistId.mockResolvedValue(
      updatedWishlistItemsDB
    );
  ...
  test("should throw NotFoundError if wishlist does not exist", async () => {
    ...
    await expect(wishlistBusiness.updateWishlist(input)).rejects.toThrow(
      NotFoundError
    );
  });
