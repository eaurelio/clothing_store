...
import { UpdateOrderInputDTO, UpdateOrderOutputDTO } from '../../../src/dtos/orders/updateOrder.dto';
...
describe('OrderBusiness - updateOrder', () => {
  ...
  test('should successfully update an order with items', async () => {
    ...
    mockOrderDatabase.findOrderItemsByOrderId.mockResolvedValue([
      { item_id: 'item_id_1', product_id: 'product_id_1', quantity: 2, price: 10 },
      { item_id: 'item_id_2', product_id: 'product_id_2', quantity: 3, price: 20 },
    ]);
    ...
    expect(result).toEqual({
      ...
      order: {
        ...
        trackingCode: 'new_tracking_code',
        items: [
          { itemId: 'item_id_1', productId: 'product_id_1', quantity: 2, price: 10 },
          { itemId: 'item_id_2', productId: 'product_id_2', quantity: 3, price: 20 },
        ],
      },
    });
    ...
    expect(mockOrderDatabase.deleteOrderItemsByOrderId).toHaveBeenCalledWith('order_id');
  ...
  test('should throw NotFoundError if the order does not exist', async () => {
    ...
    await expect(orderBusiness.updateOrder(input)).rejects.toThrow(NotFoundError);
  });
