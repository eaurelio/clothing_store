...
import { CreateWishListInputDTO, CreateWishListOutputDTO } from '../../../src/dtos/wishlist/createWishList.dto';
...
describe('WishlistBusiness - createWishlist', () => {
  ...
  test('should create a new wishlist successfully', async () => {
    ...
    expect(mockWishlistDatabase.insertWishlist).toHaveBeenCalledWith(expectedWishlistDB);
    expect(mockWishlistDatabase.insertWishlistItem).toHaveBeenCalledWith(expectedWishlistItemDB);
  });
  ...
  test('should update an existing wishlist and remove old items', async () => {
    ...
    mockWishlistDatabase.findWishlistByUserId.mockResolvedValue(existingWishlist);
