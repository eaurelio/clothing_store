...
import { NotFoundError, ForbiddenError, BadRequestError } from '../../../src/errors/Errors';
...
describe('UserBusiness - changePassword', () => {
  ...
  test('should successfully change the password', async () => {
    ...
    mockHashManager.compare
      ...
      .mockResolvedValueOnce(false); // New password is different from old password
    ...
    expect(mockUserDatabase.updatePassword).toHaveBeenCalledWith('existing_user_id', 'hashed_new_password');
  });
  ...
  test('should throw NotFoundError if user does not exist', async () => {
    ...
    await expect(userBusiness.changePassword(input)).rejects.toThrow(NotFoundError);
  });
  ...
  test('should throw ForbiddenError if user account is deactivated', async () => {
    ...
    await expect(userBusiness.changePassword(input)).rejects.toThrow(ForbiddenError);
  });
  ...
  test('should throw BadRequestError if email does not match', async () => {
    ...
    await expect(userBusiness.changePassword(input)).rejects.toThrow(BadRequestError);
  });
  ...
  test('should throw BadRequestError if old password is incorrect', async () => {
    ...
    mockHashManager.compare.mockResolvedValueOnce(false); // Old password does not match
  ...
  test('should throw BadRequestError if new password is the same as old password', async () => {
    ...
    mockHashManager.compare
      .mockResolvedValueOnce(true) // Old password matches
      .mockResolvedValueOnce(true); // New password is the same as old password
