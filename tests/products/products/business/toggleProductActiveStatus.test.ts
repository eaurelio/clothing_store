...
import { ToggleProductActiveStatusInputDTO, ToggleProductActiveStatusOutputDTO } from '../../../src/dtos/products/updateProduct.dto';
...
describe('ProductBusiness - toggleProductActiveStatus', () => {
  ...
  test('should successfully activate a product', async () => {
    ...
    const result: ToggleProductActiveStatusOutputDTO = await productBusiness.toggleProductActiveStatus(input);

    expect(result).toEqual({
      message: 'Product activated successfully',
    });
    expect(mockProductDatabase.updateProductActiveStatus).toHaveBeenCalledWith('product_id', true);
  });
  ...
  test('should successfully deactivate a product', async () => {
    ...
    expect(mockProductDatabase.updateProductActiveStatus).toHaveBeenCalledWith('product_id', false);
  });
  ...
  test('should throw NotFoundError if product does not exist', async () => {
    ...
    await expect(productBusiness.toggleProductActiveStatus(input)).rejects.toThrow(NotFoundError);
  });
});
