...
describe('TicketBusiness - updateTicket', () => {
  ...
  test('should update a ticket successfully', async () => {
    ...
    expect(mockTicketDatabase.findTicketById).toHaveBeenCalledWith(input.ticketId);
    expect(mockTicketDatabase.updateTicket).toHaveBeenCalledWith(updatedTicketDB);
  });
