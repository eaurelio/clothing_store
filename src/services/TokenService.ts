...
class TokenService {
  ...
  // Gera um token JWT com um payload simples
  public generateToken(userId: string, role: string): string {
    try {
      const payload = {
        userId,
        role,
        createdAt: Date.now(),
        expiresAt: Date.now() + (7 * 24 * 60 * 60 * 1000) // 7 dias em milissegundos
      };
  ...
  // Verifica a validade e a assinatura do token JWT
  public verifyToken(token: string): { userId: string, role: string } | null {
    try {
      const [base64Payload, signature] = token.split('.');

      // Recria a assinatura para comparar com a assinatura fornecida
      const newSignature = crypto.createHmac('sha256', this.secret)
        .update(base64Payload)
        .digest('base64');
